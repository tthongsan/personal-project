...
class MensUnderArmour extends Component {
  constructor () {
    super();
    this.state = {
      underarmour: []
...
getUnderArmour() {
  axios.get(`/api/product/${'underarmour'}`).then(res => {
    //console.log(res.data[0].shoes_name)
    console.log(res.data)
    this.setState({underarmour: res.data})
  })
}
...
addToCart = (name, price, image, id) => {
 ...
 let mappedShoes = this.state.underarmour && this.state.underarmour.map(shoe => {
...
export default connect(mapStateToProps)(MensUnderArmour);