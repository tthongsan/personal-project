...
class WomensConverse extends Component {
  constructor () {
    super();
    this.state = {
      wconverse: []
...
componentDidMount() {
  this.getWomensConverse();
}


getWomensConverse() {
  axios.get(`/api/product/${'wconverse'}`).then(res => {
    //console.log(res.data[0].shoes_name)
    console.log(res.data)
    this.setState({wconverse: res.data})
  })
}
...
addToCart = (name, price, image, id) => {
 ...
 let mappedShoes = this.state.wconverse && this.state.wconverse.map(shoe => {
