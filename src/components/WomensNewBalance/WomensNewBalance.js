...
class WomensNewBalance extends Component {
  constructor () {
    super();
    this.state = {
      wnewbalance: []
...
componentDidMount() {
  this.getWomensNewBalance();
}


getWomensNewBalance() {
  axios.get(`/api/product/${'wnewbalance'}`).then(res => {
    //console.log(res.data[0].shoes_name)
    console.log(res.data)
    this.setState({wnewbalance: res.data})
  })
}
...
addToCart = (name, price, image, id) => {
 ...
 let mappedShoes = this.state.wnewbalance && this.state.wnewbalance.map(shoe => {
...
export default WomensNewBalance;